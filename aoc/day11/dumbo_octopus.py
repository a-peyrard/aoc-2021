...
def count_flashes(matrix: List[List[int]], nb_steps: int) -> int:
    flashes = 0
    for step in range(nb_steps):
        flashes += _count_flashes_for_step(matrix)
        DEBUG and print_matrix(matrix, step + 1)

    return flashes


def _count_flashes_for_step(matrix: List[List[int]]) -> int:
    flashes_counter = 0
    flashes_stack: List[Coordinate] = []
    for y, row in enumerate(matrix):
        for x, val in enumerate(row):
            matrix[y][x] = (val + 1) % 10
            if matrix[y][x] == 0:
                flashes_counter += 1
                flashes_stack.append(Coordinate(x=x, y=y))

    while len(flashes_stack) > 0:
        flash = flashes_stack.pop()
        tmp_counter, tmp_stack = _apply_flash(flash, matrix)
        flashes_counter += tmp_counter
        flashes_stack.extend(tmp_stack)

    return flashes_counter


def _apply_flash(flash: Coordinate,
                 matrix: List[List[int]]) -> Tuple[int, List[Coordinate]]:
    flashes_counter = 0
    flashes_stack: List[Coordinate] = []
    for y in range(max(0, flash.y - 1), min(len(matrix), flash.y + 2)):
        for x in range(max(0, flash.x - 1), min(len(matrix[y]), flash.x + 2)):
            if (y != flash.y or x != flash.x) and matrix[y][x] != 0:
                matrix[y][x] = (matrix[y][x] + 1) % 10
                if matrix[y][x] == 0:
                    flashes_counter += 1
                    flashes_stack.append(Coordinate(x=x, y=y))

    return flashes_counter, flashes_stack


def print_matrix(matrix: List[List[int]], step: int):
    print(f'matrix at step {step}: \n')
    for row in matrix:
        print("".join(map(str, row)))
    print('\n')


def find_all_octopus_flashes(matrix: List[List[int]]) -> int:
