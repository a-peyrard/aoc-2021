...
def count_lanternfish(fishes: Iterable[int], days: int = 80) -> int:
    number_of_fishes = 0
    new_fishes_per_day = [0] * 7
    for fish in fishes:
        new_fishes_per_day[fish] += 1
        number_of_fishes += 1

    creation_buffer = [0] * 2
    for day in range(days):
        day_of_week = day % 7
        fishes_to_create = new_fishes_per_day[day_of_week]
        new_fishes_per_day[day_of_week] += creation_buffer[0]
        creation_buffer[0] = creation_buffer[1]
        creation_buffer[1] = fishes_to_create
        number_of_fishes += fishes_to_create
