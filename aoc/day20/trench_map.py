...
multiple lines in this example for legibility. The second section is the input image, a two-dimensional grid of light pixels (#) and dark pixels (.).
...
def count_pixels(algo: List[bool],
                 image: Set[Tuple[int, int]],
                 x_min: int,
                 width: int,
                 y_min: int,
                 height: int,
                 number_of_steps: int = 2) -> int:
    image, _, __, ___, ____ = enhance_image(algo, image, x_min, width, y_min, height, number_of_steps)

    return len(image)


def enhance_image(algo: List[bool],
                  image: Set[Tuple[int, int]],
                  x_min: int,
                  width: int,
                  y_min: int,
                  height: int,
                  number_of_steps: int = 2) -> Tuple[Set[Tuple[int, int]], int, int, int, int]:
    for step in range(number_of_steps):
        image, x_min, width, y_min, height = \
            _enhance_image_step(algo, image, x_min, width, y_min, height, (step + 1) % 2 == 0)

    return image, x_min, width, y_min, height


def _enhance_image_step(algo: List[bool],
                        image: Set[Tuple[int, int]],
                        x_min: int,
                        width: int,
                        y_min: int,
                        height: int,
                        even_step: bool) -> Tuple[Set[Tuple[int, int]], int, int, int, int]:
    x_new_min = x_min - 1
    new_width = width + 2
    y_new_min = y_min - 1
    new_height = height + 2
    ...
    new_image = set()
    for y in range(y_new_min, y_new_min + new_height):
        for x in range(x_new_min, x_new_min + new_width):
            if is_a_new_light_pixel(x, y, algo, image, x_min, width, y_min, height, even_step):
                new_image.add((x, y))

    return new_image, x_new_min, new_width, y_new_min, new_height


def is_a_new_light_pixel(px: int,
                         py: int,
                         algo: List[bool],
                         image: Set[Tuple[int, int]],
                         x_min: int,
    ...
    for y in range(py - 1, py + 2):
        for x in range(px - 1, px + 2):
            light_pixel = 1 if _is_a_light_pixel(x, y, algo, image, x_min, width, y_min, height, even_step) else 0
            pos = (pos << 1) | light_pixel

    return algo[pos]


def _is_a_light_pixel(px: int,
                      py: int,
                      algo: List[bool],
                      image: Set[Tuple[int, int]],
                      x_min: int,
                      ...
                      height: int,
                      even_step: bool) -> bool:
    if x_min <= px < x_min + width and y_min <= py < y_min + height:
        return (px, py) in image

    if not algo[0] or not even_step:
        return False
...
def draw_image(image: Set[Tuple[int, int]],
    ...
    for y in range(y_min, y_min + height):
        for x in range(x_min, x_min + width):
            image_str += "#" if (x, y) in image else "."
        image_str += "\n"
