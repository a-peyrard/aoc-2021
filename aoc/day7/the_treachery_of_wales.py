...
def calculate_amount_of_fuel(positions: Iterable[int]) -> int:
    sorted_positions = sorted(positions)
    chosen_position = sorted_positions[int(len(sorted_positions) / 2)]

    return sum((abs(p - chosen_position) for p in sorted_positions))


def calculate_amount_of_fuel_2(positions: Iterable[int]) -> int:
    positions = list(positions)
    raw_pivot = statistics.mean(positions)
    pivot1 = ceil(raw_pivot)
    pivot2 = int(raw_pivot)

    sum1 = sum((_fuel(p, pivot1) for p in positions))
    sum2 = sum((_fuel(p, pivot2) for p in positions))

    return min(sum1, sum2)


def _fuel(position: int, pivot: int) -> int:
