...
Examples of corrupted chunks include (], {()()()>, (((()))}, and <([]){()}[{}]). Such a chunk can appear anywhere wi
thin a line, and its presence causes the whole line to be considered corrupted.
...
def calculate_corrupted_score(lines: List[str]) -> int:
    return sum((
        _calculate_corrupted_score_for_line(line)
        for line in lines
    ))
